...
export const renderSelectRelationship = ({ name, label, options, state, onChange }) => {
  ...
  return (
    <TextField
      ...
      onChange={onChange}>
      {options[0].map((item) => (
        <MenuItem key={(item || "").codeValue} value={(item || "").codeValueId}>
          {(item || "").codeValue}
        </MenuItem>
      ))}
    </TextField>
...
export const renderSelectGender = ({ name, label, options, state, onChange }) => {
